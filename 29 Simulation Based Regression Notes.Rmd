...
Bootstrapping residuals corresponds to a designed experiment, where the $x$ values are fixed and only $Y$ is random. If we bootstrap observations, then in essentially both $X$ and $Y$ are sampled. By the principle of sampling the way the data were drawn, the second method implies that both $Y$ and $X$ are random.
...
Nice. To resample the data we use `do()` with `resample()`. This will sample the rows, what we were refering to above as the first method.
...
With all this data in `results`, we can generate confidence intervals for the slope, $R$-squared, and $F$.
...
confint(star_mod)
```
The bootstrap confidence interval for $R$-squared is:

```{r}
cdata(~r.squared,data=results)
```

And the bootstrap sampling distribution of $R$-squared is:
...
This matches the work we have already done. Finally, let's visualize the results.
...
We could also resample the residuals instead of the data. This makes a stronger assumption that the linear model is appropriate. However, it guarantees that every $X$ value is in the resample dataframe. In the `lm` function, we send the model instead of the data to resample the residuals.
...
First as a reminder, the prediction interval at 60 grams of `carb` using the assumption of normally distributed errors from last lesson is:
...
Close, just a little bit different as it is skewed to the right of what we got last lesson.
...
When the covariate takes the value A, $\E(Y|X=A)=\beta_0 + \beta_1$. 

When the covariate takes the value B, $\E(Y|X=B)=\beta_0 + \beta_2$. 

When the covariate takes the value C, $\E(Y|X=C)=\beta_0$. 
...
The `Std.Error`, `t value`, and `Pr(>|t|)` values can be used to conduct inference about the respective estimates. Both p-values are significant. This is similar to the ANOVA analysis we conducted last block except that hypothesis test was simulataneously testing all coefficents and here we are testing them pairwise. 
