...
Often probability models are used to answer a question of the form "What is the chance that .....?" This means that we typically have an experiment or trail where multiple outcomes are possible and we only have an idea of the frequency of those outcomes. We use this frequency as a measure of the probability of a particular outcome.
...
1. What is the experiment or trail?
...
The answer for 18 people is $365 \times 364 \times 363 ... \times 349 \times 348$. This looks like a truncated factorial. Remember a factorial, written as $n!$ with an explanation point, is the product of successive positive integers. As an example $3!$ is $3 \times 2 \times 1$ or 6. We could write this as $$\frac{365!}{(365-n)!}$$. As we will learn, this is known as a **permutation**.

We are ready to put it back together again. For 18 people, the probability of 2 or more people with the same birthday is 1 minus the probability that no one has the same birthday, which is

$$1 - \frac{\frac{365!}{(365-18)!}}{365^{18}}$$ or 

$$1 - \frac{\frac{365!}{347!}}{365^{18}}$$

In `R` there is a function called `factorial()` but factorials get large fast and we will **overflow** the memory. Try `factorial(365)` in `R`

```{r}
factorial(365)
```

It is returning *infinity* because the number is too large for the buffer. As is often the case we will have in using a computational method, we will have to be clever about our approach. Instead of using factorials we can make use of `R` ability to work on vectors. If we provide `R` with a vector of values, the `prod()` will perform a product of all the elements.
...
We now have the mathematics to understand the problem. We can easily generalize this to any number of people. To do this, we have to write a function in `R`. As in everything in `R` we save a function as an object. The general format for creating a function is
...
Notice we assigned the function to the name `birthday_prob`, we told `R` to expect one argument to the function, which we are calling `n`, and then having `R` find the probability. We set a default value for `n` in case one is not provided to prevent an error. We will learn more about writing functions over this and next semester.
...
Let's create a plot of the probability versus number of people in the room. To do this, we need to apply the function to a vector of values. The function `sapply()` will work or we can also use `Vectorize()`. We will choose the later.
...
We are good to go. Let's create our plot.
...
It could be argued that we could randomly pick one year and use it. Let's see what happens if we just used 1969.
...
> **Exercise**:  
What patterns do you see in the data? What might explain them?

There are definitely bands appearing in the data which could be the day of the week, less birthdays on the weekend. There is also seasonality with more birthdays in the summer and fall. There is also probably an impact from holidays. 

Quickly, let's look at the impact of day of the week.
...
By only using one year, this data might give poor results. So we will combine but we also still have the problem of leap years.
...
Let's look at a plot of the number of births versus day of the year.
...
We use these numbers as weights. Days with more births will have a higher probability of being selected. Days such as Christmas and Christmas eve has a lower probability of being selected. Let's save the weights in an object to use in the `sample()` function.
...
Now let's simulate, the probability of a match should change slightly, maybe go down slightly?, but not much since most of the days have about the same probability or number of occurrences.
...
We could not solve this problem using mathematics.
