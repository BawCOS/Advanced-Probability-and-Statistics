...
Let's create a plot of the probability versus number of people in the room. To do this, we need to apply the function to a vector of values. The function `sapply()` will work or we can also use `Vectorize()` to alter our existing function. We choose the later option.
...
Is this what you expected the curve to look like? We did not expect this. It has a sigmodial shape with a large increase in the middle range and flatten in the tails.
...
By only using one year, this data might give poor results. So we will combine years. Note that we also still have the problem of leap years.
...
Cool stuff, let's get to learning more about probability models.
