...
There are least two outliers in the plot. Let's gather more information about them.
...
There is relatively little change in the adjusted $R^2$, so we will go with this model. Again, Math 378 will go more into depth about model selection.
...
To make this this section more relevant, we are going to switch to a different data set. Load the library `ISLR`, this is a package that you will use a great deal in Math 378.
...
In this lesson we have extended the linear regression model by allowing multiple predictors. This allows to account for confounding variables and make more sophisticated models. The interpretation and evaluation of the model changes.
