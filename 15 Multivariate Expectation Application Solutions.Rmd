...
$$
\rho=\frac{\Cov(X,Y)}{\sqrt{\Var(X)\Var(Y)}}=\frac{-0.007}{\sqrt{0.076\times0.076}}=-0.909
$$
...
$$
\renewcommand{\arraystretch}{1.1} \begin{array}{cc|cccccc} & & \multicolumn{6}{c}{\mbox{X}}\\ 
& & 1 & 2 & 3 & 4 & 5 & 6 \\
\cline{2-8}
\multirow{6}{*}{\mbox{Y}} & 1 & \frac{1}{36} & \frac{1}{36} & \frac{1}{36} & \frac{1}{36} & \frac{1}{36} & \frac{1}{63} \\
...
Find $E[XY]$ so we just go through an find all 36 values of the product and multiply by the probabilities. Since the probabilities are all constant, we will take that out. Now
...
Let's write a function that takes a vector and returns the sum of the values up to the first time 2 appears. Anytime you are repeating something more than 5 times, it might make sense to write a function.
