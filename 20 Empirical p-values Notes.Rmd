...
```
Note: I could have picked the lower right cell as the reference cell. But now I want the $\Prob(X \geq 14)$ with the appropriate change in parameter values. Notice we get the same answer.
...
The p-value is slightly different since the **hypergeometric** is not symmetric. Doubling the p-value from the single side result is not quite right. The algorithm in `fisher.test()` finds and adds all probabilities less than or equal to value of $\Prob(X = 11)$. This is the correct p-value.
...
\item[$H_A$:] There is some other distribution of percentages in the population. At least one population proportion is not $\frac{1}{4}$.
\end{itemize}
