...
1) Conduct a hypothesis test, all four steps, using probability models.
...
This decision region looks like the pmf below, any observed value in the red lines would be consistent with the null hypothesis. Any value at the red or more extreme would be in the rejection region.
...
In these 10,000 simulations, we see vary few results close to 0.35.
...
Out last example will be interesting because the distribution has multiple parameters and a test metrics is not obvious at this point.
...
We will use the `sample()` function again. We are only using a sample size of 12 to keep the printout reasonable.
...
Notice this is not use `tidyverse` coding ideas. We don't think we need tibbles or dataframes so we went straight nested `R` code. You can break this code down by starting with the code in the center.
...
Let's simulated the data under the null hypothesis.
...
The p-value is 0.14, this value is not extreme in either direction. The residents may want to repeat the study with more data.
...
If each number we equally likely, we would have 121.5 balls in each bin. We can find a test statistic by looking at the deviation from 121.5.
...
We will similar code from above with our new metric.
...
The p-value is 0.014, this value is much smaller than our previous result. The test statistic matters as nothing about this problem has changed except the test statistic.
