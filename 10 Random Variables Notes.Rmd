---
title: "Discrete Random Variables Notes"
author:
- Lt Col Ken Horton
...
2) Given a discrete random variable, describe probability associated with each outcome of the variable.

3) Given a discrete random variable, obtain the pmf and cdf, and use them to obtain probabilities of events.

4) Simulate random variable for a discrete distribution. 
...
Let the random variable $X$ be the number of heads in three coin flips. Whenever introduced to a new random variable, you should take a moment to think about the domain of that variable. What possible values can $X$ take? When tossing a coin 3 times, we can get no heads, one head, two heads or three heads. The random variable $X$ assigns each outcome in our experiment to one of these values. Visually: 
$$
...
Because the domain (or support) of $X$ is a countable list of numbers, we consider $X$ to be a *discrete* random variable (more on that later). 

### How Does This Help?

Sticking with our example, we can now frame a problem of interest in the context of our random variable $X$. For example, suppose we wanted to know that probability of at least two heads. Without our random variable, we have to write this as:
$$
...
A *discrete* random variable has a domain that consists of a countable range of values. $X$ in our example above is a discrete random variable. Note that "countable" does not necessarily mean "finite". For example, a random variable with a Poisson distribution (a topic for a later lesson) has a domain of $\{0,1,2,...\}$. This domain stretches to infinity, but the domain is considered *countably* infinite, and thus the random variable would be considered discrete. 
...
A *mixed* random variable is exactly what it sounds like. It has a domain that is both discrete and continuous. How could such a thing occur? Consider an experiment where a person rolls a standard six-sided die. If it lands on anything other than one, the result of the die roll is recorded. If it lands on one, the person spins a wheel, and the angle in degrees of the resulting spin, divided by 360, is recorded. If our random variable $Z$ is the number that is recorded in this experiment, the domain of $Z$ is $[0,1] \cup \{2,3,4,5,6\}$. We will not be spending much time on mixed random variables. 


### Discrete Distribution Functions

Once we have defined a random variable, we need a way to describe its behavior. In other words, how is that variable *random*? 
...
Note that the pmf is a *function*. Functions have input and output. The input of a pmf is any real number. The output of a pmf is the probability that the random variable takes the inputted value. The pmf must follow the axioms of probability described in lesson 2. Primarily,
...
Graphically, the pmf is not terribly interesting. The pmf is 0 at all values of $X$ except for 0, 1, 2 and 3. 

```{r echo=FALSE, fig.height=3, fig.width=4, fig.cap="Probability Mass Function of $X$ from Example 6.1"}
...
Visually, the cdf of a discrete random variable has a stairstep appearance. In this example, the cdf takes a value 0 up until $X=0$, at which point the cdf increases to 1/8. It stays at this value until $X=1$, and so on. At and beyond $X=3$, the cdf is equal to 1. 

```{r lesson6b, echo=FALSE, fig.height=3, fig.width=4, fig.cap="Cumulative Distribution Function of $X$ from Example 6.1"}
x<-c(0,1,2,3)
y<-c(0,1/8,1/2,7/8)
y2<-c(1/8,1/2,7/8,1)
...
We can simulate values from a random variable using the cdf, we will use a similar idea for continuous random variables. Since range is in the interval $[0,1]$ we will generate a random number in that same interval and then use the inverse function to find the value of the random variable. The pseudo code is:  
...
From our simulation above, we can find the mean as an estimate of the expected value.
