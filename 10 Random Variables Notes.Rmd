...
```{r echo=FALSE, fig.height=3, fig.width=4, fig.cap="Probability Mass Function of $X$ from Coin Flip Example"}
...
Visually, the cdf of a discrete random variable has a stairstep appearance. In this example, the cdf takes a value 0 up until $X=0$, at which point the cdf increases to 1/8. It stays at this value until $X=1$, and so on. At and beyond $X=3$, the cdf is equal to 1. See the figure below, Figure 2. 

```{r lesson6b, echo=FALSE, fig.height=3, fig.width=4, fig.cap="Cumulative Distribution Function of $X$ from Coin Flip Example"}
x<-c(0,1,2,3)
y<-c(0,1/8,1/2,7/8)
y2<-c(1/8,1/2,7/8,1)
...
From our simulation above, we can find the mean as an estimate of the expected value, this really a statistic since our simulation is data from the population and thus will have variance from sample to sample.
