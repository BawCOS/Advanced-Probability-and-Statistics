---
title: "Multivariate Distribution Applications Solutions"
author:
- Lt Col Ken Horton
- Lt Col Kris Pruitt
...
$$
\Prob(X<0.2|Y\geq 0.75)=\frac{\Prob(X<0.2,Y\geq 0.75)}{\Prob(Y\geq 0.75)}=\frac{0.06875}{\int_{0.75}^1 y+\frac{1}{2}\diff y}=\frac{0.06875}{0.34375}=0.2
$$
...
$$
$$
=\frac{1}{3}\left( 2x^2 +x \right) \bigg|_0^{0.2} = \frac{1}{3}\left( 2\cdot0.2^2 +0.2 \right) = 0.0084
$$
...
$$
\Prob(X\leq Y)=\int_0^1\int_0^x x+y \diff y \diff x = \int_0^1 xy+\frac{y^2}{2}\bigg|_0^x \diff x = \int_0^1 \frac{3x^2}{2}\diff x = \frac{x^3}{2}\bigg|_0^1 = \frac{1}{2}
$$

&nbsp;


\indent 2. In the Notes, we saw that $f_X(x)=f_{X|Y=y}(x)$ and $f_Y(y)=f_{Y|X=x}(y)$. What does this imply about $X$ and $Y$?  

Since the conditional density function is always equal to the marginal, it means that $X$ and $Y$ are independent of one another. Also, if the conditioning variable does not appear in the conditional density function and the support is rectangular, the random variables are independent. The variables in the previous problem are dependent, look at the conditional density functions to see that the conditional density depends on the conditioned variable.
...
$$
\renewcommand{\arraystretch}{1.1} \begin{array}{cc|ccc} & & \multicolumn{3}{c}{X: \mbox{Weather Status}}\\ 
& & 0 \mbox{ (Clear)} & 1 \mbox{ (Light Rain)} & 2 \mbox{ (Rain)}  \\
\cline{2-5}
\multirow{6}{*}{Y: \mbox{num complaints}} & 0 & 0.133 & 0.052 & 0.019  \\
& 1 & 0.086 & 0.071 & 0.038  \\
& 2 & 0.081 & 0.119 & 0.057  \\
& 3 & 0.062 & 0.071 & 0.076  \\
& 4 & 0.038 & 0.038 & 0.048 \\
...
$$
\Prob(Y<3|X=0)=\frac{0.133+0.086+0.081}{0.657}=0.457
$$
