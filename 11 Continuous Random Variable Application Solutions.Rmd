---
title: "Continuous Random Variables Applications Solutions"
author:
  - Lt Col Ken Horton
...
i) This is tricky since we need a cube root function. Just raising to the one-third power won't work. Let's write our own function.
...
Let $a<b$, where $a$ and $b$ are both in the domain of $X$. Note that $F_X(a)=\Prob(X\leq a)$ and $F_X(b)=\Prob(X\leq b)$. Since $a<b$, we can partition $\Prob(X\leq b)$ as $\Prob(X\leq a)+\Prob(a < X \leq b)$. One of the axioms of probability is that it must be non-negative, so I know that $\Prob(a < X \leq b)\geq 0$. Thus, 
$$
