...
This is one of the more mathematical sections of the course. There are some ways to reduce the mathematics using computational resources but by its nature this lesson is still mathematical.
...
Using `R` to do this requires a new package `cubature`. You can install it from `RStudio` package tab or the command line using `install.packages("cubature")`.  It is installed as part of our RStudio Cloud server. Then we can use it as follows:
