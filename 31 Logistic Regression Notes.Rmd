...
First get the predicted values using `augment()` from the `broom` package. We have to convert the frist column from a logic variable into a numeric variable and changed `.fitted` into a predicted probability. If the first assumption is valid we would expect a 45 degree line which is close to what we have.
