...
Let's build the model and summarize it.
...
First get the predicted values using `augment()` from the `broom` package. We have to convert the first column from a logic variable into a numeric variable and changed `.fitted` into a predicted probability. If the first assumption is valid we would expect a 45 degree line which is close to what we have.
...
There is too much overlap on the points so let's add some noise to separate the data points just to visualize better.
...
The problem with this data is that essential all the predictors are binary. Some like `dollar` and `password` are counting variables but most of the values are zero. However, we will use the variable `passwords` to show how we can tell if the variable is linear on the logit scale.
...
First, let's use `R` the built in function `confint()` to find the confidence interval for the simple logistic regression model.
...
As a reminder, the output from out multiple logistic regression model is:   
