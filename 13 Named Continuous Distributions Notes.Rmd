...
1) Recognize when to use common continuous distributions (Uniform, Exponential, Gamma, Normal, Beta, Weibull), identify parameters, and find moments.  
...
In this lesson we will explore continuous distributions. This means we work with probability density functions and use them to find probabilities. Thus we must integrate, either numerical, graphically, or mathematically. The cumulative distribution function will also play an important role in this lesson. 
...
The mean of $X$ is $\E(X)=\frac{a+b}{2}$ and the variance is $\Var(X)=\frac{(b-a)^2}{12}$. The derivations of the mean is left to the Application. 

The most common uniform distribution is $U(0,1)$ which we have already used several times in this course. Again, notice that the plot of the **pdf** is a constant or uniform value.
...
Both from the figure and the mean and median, we know that the exponential distribution is skewed to the right.
...
The number of events in a box is $X\sim \textsf{Poisson}(\lambda = 5)$. As you can see, some boxes have more than 5 and some less because 5 is the average number of arrivals. Also note that the spacing is not equal. The runs are just repeated simulations of the same process. We can spacing and clusters in each run.
...
Some plots of normal distributions for different parameters are plotted in Fibure 3.
...
For various values $\alpha$ and $\beta$, the pdf of a beta distributed random variable is shown in Figure 4. 
