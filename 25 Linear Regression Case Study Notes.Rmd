...
### Creating a homework report

To create your homework report, in RStudio, go to New File -> R Markdown... Then, choose From Template and then choose `Lab Report for OpenIntro Statistics Labs` from the list of templates. Place your answers to the exercises in the lab report.
...
1.  Create a scatter plot to display the relationship between the personal
...
1.  Find the correlation between `pf_score` and `pf_expression_control`.
...
markdown document report. Running the function also requires that the `hfi` dataset is loaded 
in your environment.
</div>
...
black and the residuals in blue. Note that `pf_expression_control` only takes on 38 unique values, so the residual lines overlap each other. There are 38 residual one for each observations. Residuals are the difference between the observed values and the values predicted by the line:
...
that `R` should look in the `hfi_sub` data frame to find the two variables.
...
\[
  \hat{y} = 5.02721 + 0.41988 \times pf\_expression\_control
\]
