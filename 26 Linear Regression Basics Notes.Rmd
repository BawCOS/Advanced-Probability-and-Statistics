...
This model is often referred to as a **linear regression** model. (When there is only one predictor variable, we often refer to it as a **simple linear regression** model.) The $\beta_j$ terms are referred to as **coefficients**; note that the coefficients are **parameters**. They are Greek letters, which implies they are random variables. We typically don't know the true values of $\beta_j$, so we have to estimate them with samples from the population of $X$ and $Y$. We consider the models above to be linear models because they are linear in the **parameters**. This means that the following model is also a linear model:

$$
Y=\beta_0+\beta_1 X^2
$$

Even if we expect two random variables $X$ and $Y$ to share a linear relationship, we also expect some scatter, that is the will not typically be a perfect linear relationship between the response and predictor(s). For example, consider the head length and total length of 104 brushtail possums from Australia. The data is in the file `possum.csv` in the data folder. 

The head and total length variables are associated. Possums with an above average total length also tend to have above average head lengths. To visual this data, we will use a scatterplot, we have used these multiple times this semester. Scatterplots are a graphical technique to present two numerical variables simultaneously. Such plots permit the relationship between the variables to be examined with ease. The following figure shows a scatterplot for the head length and total length of the possums. Each point represents a single possum from the data. First we will read in the data, convert character variables to factors, and then plot a scatter plot.
...
Let's change the variables `pop` and `sex` into factors.
