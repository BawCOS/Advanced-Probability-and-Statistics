...
The table below shows 5 rows of the `email50` data set concerning 50 emails from 2012. The data object `email50` is a subset of `email` and we have selected to only list 5 rows and 5 variables for ease of observation. 
...
```{r, echo=FALSE,fig.dim=c(5,3), fig.cap="A scatterplot showing fed_spend against poverty. Owsley County of Kentucky, with a poverty rate of 41.5% and federal spending of $21.50 per capita, is highlighted."}
...
```{r echo=FALSE,fig.dim=c(5,3),fig.cap="A scatterplot of the homeownership rate versus the percent of units that are in multi-unit structures for all 3,143 counties."}
pch <- 1
...
In this section we will create a simple scatterplot and then ask you to create one on your own. First we will recreate the scatterplot seen in Figure 2. This figure uses the `county_M377` data set.
