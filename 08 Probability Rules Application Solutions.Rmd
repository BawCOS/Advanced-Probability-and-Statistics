---
title: "Probability Rules Applications Solutions"
author:
- Lt Col Ken Horton
...
$$
\Prob(\mbox{at least one other person shares my bday})=1-\Prob(\mbox{no one else has my bday}) = 1-\frac{364}{365}^{20} = 0.0534
$$
...
Generalizing,
$$
\Prob(\mbox{at least one other person shares my bday})=1-\Prob(\mbox{no one else has my bday}) = 1-\frac{364}{365}^{n}
$$
```{r}
n<-1:80
...
6. Consider party example in the reading. 
...
c) What is the probability of drawing a "four of a kind" (four cards of the same value)? 
...
First pick the value for the three of a kind, there are 6. Then pick the value from the remaining 5 for the two of a kind. This is actually a permuation. There are 30 distinct "flavors" of full house (three 1's & two 2's, three 1's & two 3's, etc.). In the reading we did this as 
$$
